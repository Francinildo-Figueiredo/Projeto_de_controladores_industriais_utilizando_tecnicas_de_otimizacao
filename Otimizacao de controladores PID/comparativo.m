...
lb, ub, @(x1)confun(x1,s,G,MS_max,MT_max,Ju_max), options);
...
% Critérios após a otimização
Jv_ga=norm(feedback(pade(G)/s,Kpid3),inf)
Ju_ga=norm(feedback(Kpid3, pade(G)),inf)
MS_ga=norm(feedback(1,pade(G)*Kpid3),inf)
MT_ga=norm(feedback(pade(G)*Kpid3,1),inf)
[Gm_ga,Pm_ga] = margin(G*Kpid3)

t = 0:0.1:10;
y1 = step(H1, t);
ISE_SIMC = sum((1-y1).^2)
y2 = step(H2, t);
ISE_fmincon = sum((1-y2).^2)
y3 = step(H3, t);
ISE_ga = sum((1-y3).^2)
step(H2, 'k'); hold on;
step(H3, 'y');
% title('Planta G_1');
legend('SIMC', 'fmincon', 'GA', 'best','FontSize', 10);
grid on;
